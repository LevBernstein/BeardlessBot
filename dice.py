...
def roll(message): # takes a string of the format !dn+b and rolls one n-sided die with a modifier of b.
    ...
    if command.startswith("4") or command.startswith("6") or command.startswith("8"):
        return randint(1, int(command[0])) + modifier * int(command[2:]) if len(command) != 1 and (command[1] =="+" or command[1] == "-") else randint(1, int(command[0]))
    elif command.startswith("100"):
        return randint(1, 100) + modifier*int(command[4:]) if len(command) != 3 and (command[3] == "+" or command[3] == "-") else randint(1, 100)
    elif command.startswith("10") or command.startswith("12") or command.startswith("20"):
        return randint(1, int(command[:2])) + modifier * int(command[3:]) if len(command) != 2 and (command[2] == "+" or command[2] == "-") else randint(1, int(command[:2]))
    return None